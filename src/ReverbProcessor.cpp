...
void ReverbProcessor::updateParameters(float wet, float feedback, float lpFreq) {
    ReverbProcessorVerb.SetFeedback(0.2f + constrain(feedback, 0.f, 1.f) * .4f);
    // ReverbProcessorVerb.SetLpFreq(constrain(lpFreq, 0.f, 1.f) * 20000.0f);

    // wetMix = constrain(wet, 0.f, 1.f);
}
